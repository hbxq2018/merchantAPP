...
export default {
  install(Vue) {
    ...
    Vue.prototype.createElement = function () {
      ...
      str += '<div class="crop_loading"><div class="crop_content">图片上传中...</div></div></div>';
    ...
    Vue.prototype.clip = function (e, opt) {
      ...
      let suffixIndex = loadfileName.lastIndexOf(".");
      let suffix = loadfileName.substring(suffixIndex + 1).toUpperCase();
    ...
    Vue.prototype.postImg = function (imageData) {
      ...
      $.ajax({
        ...
        success: function (res) {
          loadUrl = res.data.picUrl;
          self.resultObj.src = loadUrl;
          window.setTimeout(function () {
            document.querySelector('.crop_loading').style.display = 'none';
            document.querySelector('.crop_success').style.display = 'block';
            //裁剪完后摧毁对象
    ...
    Vue.prototype.base64ToBlob = function (imageData) {
      let self = this;
